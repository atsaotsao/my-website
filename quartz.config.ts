...
function setupImages() {
  if (document.body.dataset.slug !== "" && document.body.dataset.slug !== "index") {
    return;
  }
  ...
  images.forEach(img => {
    if (img.src.includes('light-mode')) lightImg = img;
    if (img.src.includes('dark-mode')) darkImg = img;
  });
  
  if (!lightImg || !darkImg) {
    alert('Missing images. Light: ' + !!lightImg + ', Dark: ' + !!darkImg);
    return;
  }
  
  // Force identical styling
  [lightImg, darkImg].forEach(img => {
    img.style.cssText = 'margin: 0 0 1rem 0 !important; padding: 0 !important; width: 100% !important; height: auto !important; border: none !important; border-radius: 0 !important; box-shadow: none !important; position: relative !important; display: block !important; max-width: 100% !important;';
  });
  ...
  new MutationObserver(updateImages).observe(document.documentElement, {
    attributes: true,
    attributeFilter: ['saved-theme']
  });
  
  alert('Image switching setup complete');
}

setTimeout(setupImages, 2000);
